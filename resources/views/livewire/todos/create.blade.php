...
<div>
    @if(session('status'))
        <x-mary-alert icon="o-information-circle" class="alert-success mb-5">
            {{session('status')}}
            <x-slot:actions>
                <form wire:submit="closeAlert">
                    <x-mary-button type="submit" label="Dismiss" />
                </form>
    ...
    <form wire:submit="createTodo">
        ...
        <x-mary-button type="submit" label="Submit" class="btn-outline float-right" />
    </form>
    <div>
